let userData = {
        ...
        function renderizarGrafico() {
            ...
            if (userData.investimentos.length === 0) {
                container.innerHTML = `
                    <div class="sem-dados-grafico">
                        ...
                        <p>Comece a investir para ver seu progresso aqui!</p>
                    </div>
                `;
                return;
            }
            ...
            userData.investimentos.forEach(inv => {
                const data = new Date(inv.data + 'T00:00:00');
                const mesAno = data.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' });
                
                if (!investimentosPorMes[mesAno]) {
                    investimentosPorMes[mesAno] = 0;
                }
                investimentosPorMes[mesAno] += inv.valor;
            });

            // Pegar os últimos 6 meses/períodos
            const meses = Object.keys(investimentosPorMes).slice(-6);
            const valores = meses.map(mes => investimentosPorMes[mes]);
            const maxValor = Math.max(...valores, userData.metaTotal / 6);

            container.innerHTML = `
                <div class="grafico-barras">
                    ${valores.map((valor, index) => {
                        const altura = (valor / maxValor) * 100;
                        const corBarra = valor >= (userData.metaTotal / 6) 
                            ? 'linear-gradient(to top, #4CAF50, #45a049)' 
                            : 'linear-gradient(to top, #667eea, #764ba2)';
                        
                        return `
                            <div class="barra-container">
                                <div class="barra" style="height: ${altura}%; background: ${corBarra};">
                                    <div class="barra-valor">${formatMoney(valor)}</div>
                                </div>
                                <div class="barra-label">${meses[index]}</div>
                            </div>
                        `;
                    }).join('')}
                </div>
